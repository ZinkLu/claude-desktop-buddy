...
#include "stats.h"
// X-Knob port: no AXP192; battery/USB voltage not readable. status command
// below returns zeros for those fields instead of hardware readings.
...
inline bool xferCommand(JsonDocument& doc) {
  ...
  if (strcmp(cmd, "status") == 0) {
    ...
    // X-Knob has no AXP192 — battery/USB readings unavailable in Phase 1.
    int vBat = 0, iBat = 0, vBus = 0, pct = 0;
    char b[320];
